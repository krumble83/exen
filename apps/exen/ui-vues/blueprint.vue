		...
		<div class="panel header flexChild">
			<uimenu class="horizontal" tabindex="-1" ref="menu">
				<uimenuitem title="Project">
					<uimenu>
						<uimenuitem title="New..." shortcut="ctrl+N" action="newProject" desc="create new project" icon="apps/exen/ui-img/16x16/new.png" />
						<uimenuitem title="Open.." action="openProject" desc="open existing project" icon="apps/exen/ui-img/16x16/folder.png" />
						<uimenuitem class="sep"></uimenuitem>
						<uimenuitem title="Save" action="saveProject" desc="save current project" icon="apps/exen/ui-img/16x16/save.png" />
						<uimenuitem title="Save As..." action="saveProject" desc="save current project" icon="apps/exen/ui-img/16x16/save.png" />
						<uimenuitem class="sep"></uimenuitem>
				...
				<uimenuitem title="Edition">
					<uimenu>
						...
						<uimenuitem class="sep"></uimenuitem>
						<uimenuitem title="Preferences" action="showPreferences" icon="apps/exen/ui-img/16x16/preferences.png" />
					</uimenu>
				</uimenuitem>
				<uimenuitem title="Compile">
					<uimenu>
						<uimenuitem title="Check Project" icon="apps/exen/ui-img/16x16/check.png"></uimenuitem>
						<uimenuitem title="Compile Project" icon="apps/exen/ui-img/16x16/compile.png"></uimenuitem>
		...
		<div class="flexChild rowParent">
				...
				<div class="panel leftBottom flexChild" v-panel.bottom="20">
					...
					<div style="padding:3px;">
						<uitree 
							...
							:type="$flag('F_GRAPH')" 
							:items="graphs" 
							:button="{text: 'Add Graph', action:'addGraph', symbol: 'apps/exen/ui-img/add.png'}" 
							ref="graphsTree" 
						...
						<uitree 
							...
							:items="functions" 
							:button="{text: 'Function', action:'addFunction', symbol: 'apps/exen/ui-img/add.png'}" 
							ref="functionsTree" 
							@item:edited="onEdited"
						...
						<uitree 
							...
							:type="$flag('F_MACRO')" 
							:items="macros" 
							:button="{text: 'Macro', action:'addMacro', symbol: 'apps/exen/ui-img/add.png'}" 
							ref="macrosTree" 
							@item:edited="onEdited"
						...
						<uitree 
							...
							:items="variables" 
							:button="{text: 'Variable', action:'addVariable', symbol: 'apps/exen/ui-img/add.png'}" 
							ref="variablesTree" 
							@edited="onEdited"
			...
			<div class="flexChild columnParent">
				<div class="panel toolbar flexChild">
					<uitbbutton label="Check" img="apps/exen/ui-img/check.png" action="check" title="Check current Project" ></uitbbutton>
					<uitbbutton label="Save" img="apps/exen/ui-img/save.png" :disabled="true" action="save" title="Save current Project" ></uitbbutton>
					<uitbbutton label="F.screen" img="apps/exen/ui-img/fullscreen.png" action="switchFullscreen" :toggle="1" title="Toggle fullscreen" ></uitbbutton>
				</div>
				...
				<div class="panel footer flexChild selected" v-panel.bottom="-30">
					<infotabs ref="infoTabs">
						<div class="tab">
							...
							<label for="tabinfo_compile"><img src="apps/exen/ui-img/16x16/gear.png" style="vertical-align:bottom;padding-right:5px;" />
								<span>Compile result</span>
								<img src="img/closetab.png" style="padding-top:5px;padding-left:7px;float:right" />
							</label>
...
<script>
	...
	import uimenu from '../../../vues/contextmenu.vue';
	import uimenuitem from '../../../vues/contextmenu.item.vue';
	import uitbbutton from '../../../vues/toolbarbutton.vue';
			...
			addBlueprint: function(data, callback){
				...
				this.$nextTick(function(){
					...
					if(typeof callback == 'function')
						callback(me.$refs.tabsContainer.$children.find(item => item.name == data.name).$children[0]);
						//callback(me.$children[0].$children.find(item => item.name == data.name).$children[0]);
				})
			},
	...
	var ex = {
		...
		methods: {
			...
			openFile: function(treeItem){
				...
				if(g.tabOrder > 0){
					this.$refs.tabsContainer.select(g.name);
					return;
				}
				this.tabCount += 100;
